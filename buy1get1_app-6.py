...
SECTIONS = [
    {"label": "本檔必買",     "icon": "🛒", "url": BASE + "/campaign/life-will/best-buy/" + quote("本檔必買"),     "color": "#e60026", "bg": "#fff0f0"},
...
def scrape_section(url: str):
    ...
    # 一、活動期間
    # 優先抓 span.edm_lifeWill-dateText（class 前綴匹配）
    date_span = soup.find("span", class_=re.compile(r"dateText"))
    if date_span:
        sale_period = date_span.get_text(strip=True)
    else:
        # 備援：直接找日期文字
        dt = soup.find(string=re.compile(r"\d{4}-\d{2}-\d{2}\s*~\s*\d{4}-\d{2}-\d{2}"))
        sale_period = dt.strip() if dt else "活動期間請見官網"
        # 只保留日期部分
        m = re.search(r"\d{4}-\d{2}-\d{2}\s*~\s*\d{4}-\d{2}-\d{2}", sale_period)
        if m:
            sale_period = m.group(0)
    ...
    for h5 in soup.find_all("h5"):
        ...
        # li 清單：精準抓取 ul.Card_card-list__54xpV 底下的 <li>
        # 備援：若 class 名稱改版，fallback 到 container 內所有 <li>
        ...
        # 特價：精準抓取 div.Card_card-priceContainer 內的 p.Card_card-productPrice
        price = None
        ...
        avg_price = None
        unit = ""
        # 取 container 的所有直接文字節點（非 tag）
        all_text = container.get_text(separator="\n")
        # 找「平均」那一行後面的數字
        avg_match = re.search(r"平均([^\n]{0,10})\n(\d[\d,]*)元", all_text)
        if avg_match:
            unit = "平均" + avg_match.group(1).strip()
            avg_price = int(avg_match.group(2).replace(",", ""))
        # 若還是沒有，且有「買一送一」→ price // 2
        detail_str = " | ".join(detail_parts)
        if avg_price is None and price and "買一送一" in detail_str:
            avg_price = math.ceil(price / 2)
            unit = "平均每件"

        # 折扣率
        discount = None
        if price and avg_price and price > 0:
            discount = round(avg_price / price * 100)
    ...
    # # 二、商品清單 (改用現代化 .select() 選擇器，精準抓取 <li> 內文)
    # items = []
    
    # # 💡 直接定位畫面中所有的 h5 標籤（通常代表商品名稱標題）
    ...
    #         # 3. 特價 (productprice)：從第一個 <li> 裡面撈出含有「元」的特價數字
    #         if price is None:
    #             m = re.search(r"(\d[\d,]*)元", t)
    #             if m:
    ...
    #     # 4. 封裝進資料清單（保留原本的 key 架構，避免下游介面元件讀取不到而報錯）
    #     items.append({
    #         "name":      name,                     # 品項名稱 (title)
    #         "price":     price,                    # 特價 (productprice)
    #         "avg_price": None,                     # 💡 不要平均價格，設為 None
    #         "unit":      "",                       # 💡 不要平均單位，設為空字串
    #         "detail":    detail_str,               # 內文 (包含所有 <li> 裡面的文字)
    #         "image_url": image_url,                # 圖片 (image)
    #         "category":  _guess_category(name),    # 自動分類
    #     })
...
class PxmartApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(hdr, text="🏪",
                 font=("微軟正黑體", 22), fg_color=self.C_HDR,
                 text_color="white").pack(side="left", padx=(18, 6), pady=8)
        title_frame = ctk.CTkFrame(hdr, fg_color=self.C_HDR)
        title_frame.pack(side="left", pady=8)
        ctk.CTkLabel(title_frame, text="全聯福利中心",
                 font=("微軟正黑體", 15, "bold"),
                 fg_color=self.C_HDR, text_color="white").pack(anchor="w")
        ctk.CTkLabel(title_frame, text="特價商品瀏覽器",
                 font=("微軟正黑體", 9),
                 fg_color=self.C_HDR, text_color="#ffcccc").pack(anchor="w")
        ...
        reload_btn = ctk.CTkButton(toolbar, text="↺  重新整理",
                           font=("微軟正黑體", 11),       # 字體稍微調大一點點比較好看
                           fg_color=self.C_HDR,          # bg 變成 fg_color (按鈕背景色)
                           text_color="white",           # fg 變成 text_color (文字顏色)
                           width=100, height=32,         # 移除 padx/pady，改用 width 和 height 直接指定按鈕大小
                           cursor="hand2",
    ...
    def _on_tab_loaded(self, idx: int):
        ...
        self._refresh_table()
        self._status_var.set(
            f"「{SECTIONS[idx]['label']}」共 {n} 項  ·  雙擊商品開啟全聯官網")
    ...
    def _open_browser(self):
        ...
        try:
            ...
            try:
                ...
                pass
            webdriver.Chrome(options=opts).get(url)
            self._status_var.set(f"Selenium 已開啟：{name}")
        except Exception:
            webbrowser.open(url)
            self._status_var.set("已用系統瀏覽器開啟")
